...
File role:
    Configures Django, installed apps, authentication, database, external services, and environment-driven secrets.
...
# External integration configuration is loaded from `.env`/Docker environment.
# Secrets should stay out of Git; these settings only read them and pass safe
# values to the service layers.
...
VERTEX_AI_TIMEOUT_SECONDS = int(os.getenv("VERTEX_AI_TIMEOUT_SECONDS", "30"))
# AI generation supports both Vertex ADC and Google AI Studio/Gemini API keys.
# The service layer prefers this API-key value when present, then falls back to
...
def _optional_env(name: str, default: str = "") -> str:
    """
    Helper for the file role: Configures Django, installed apps, authentication, database, external services, and environment-driven secrets.

    `_optional_env` is kept at module level because it is reused by views/services
...
# If SMTP is requested but credentials are missing, fall back to console email
# so dummy/test registrations still succeed instead of failing the request.
